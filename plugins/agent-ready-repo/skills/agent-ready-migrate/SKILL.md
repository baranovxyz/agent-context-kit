...
Apply an approved agent-ready repository migration. Do not infer approval from the existence of an
audit or TODO list.
...
1. Use `using-agent-ready-repo`, `agent-ready-maintenance`, `tool-specific-wiring`, and
   `skill-and-agent-designer` as needed.
...
4. Prefer skills over new `commands/` files. Use `disable-model-invocation: true` for user-only
   workflows with side effects.
5. Preserve intent when moving knowledge. Do not silently delete existing project decisions.
6. Run `agent-ready-check` when available. Use `agent-ready-check --json` when the plugin is
   installed and available on `PATH`; otherwise run
   `python3 ${CLAUDE_PLUGIN_ROOT}/bin/agent-ready-check --json`.
